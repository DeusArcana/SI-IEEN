...
public class fr_Generacion_Vale_Resguardo_Bienes extends javax.swing.JFrame {
    ...
    Conexion Conect;
    private ArrayList<Object[]> l_objetos_asignados;
    ...
    public fr_Generacion_Vale_Resguardo_Bienes() throws ClassNotFoundException, IOException {
        initComponents();
        db=new Conexion();
        l_objetos_asignados=new ArrayList();
        //mostrar();
    ...
    public void cargarInventarioAsignado() throws IOException{
        datosInventario=Archivo.leerContenidoArchivo("vale_resguardo_configsB.txt").replace("$","").split(";");
    }
    
    public void cargarDatosEncargado() throws IOException{
        String datosEncargado[]=(Archivo.leerContenidoArchivo("vale_resguardo_configsA.txt")).replaceAll("$", "").split(",");
    ...
    public void mostrar() throws ClassNotFoundException{
        ...
        /*ResultSet rs =Conexion.getTabla(""
                + "SELECT empleados.nombres, user.puesto, user.area,empleados.municipio, empleados.localidad" +
                " FROM user" +
        ...
        modelo.setColumnIdentifiers(Tablas.generarColumnasGraficamenteInventarioParcial());
        try{
            /*while(rs.next()){
                modelo.addRow(datosInventario[0]);
            }*/
            tb_consulta.setModel(modelo);
        }catch(Exception e){System.out.println("Error");}
    }
    
    private void cargarDatos(String campo,String valor) throws ClassNotFoundException{
        try {
            
            conexion=db.getConexion();  
            ResultSet rs =Conect.getTabla(""
                + "SELECT empleados.nombres, user.puesto, user.area,empleados.municipio, empleados.localidad" +
                " FROM user" +
                " INNER JOIN empleados" +
                " ON user.id_empleado = empleados.id_empleado" +
            ...
            while(rs.next()){
                tf_responsable.setText(rs.getString("nombres"));
                tf_cargo.setText(rs.getString("puesto"));
                tf_area.setText(rs.getString("area"));
                tf_municipio.setText(rs.getString("municipio"));
                tf_localidad.setText(rs.getString("localidad"));
    ...
    private void initComponents() {
        ...
        tf_responsable.setEditable(false);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try{
            ...
            //Imagen de fondo
            Image imagen = Image.getInstance("img_vale_resguardo_bienes.jpg"); 
            ...
            PdfContentByte txt_ObjetosAsignados = writer.getDirectContent();
            ...
            int ini=535;
            for(int i=0;i<datosInventario.length;i++){
                System.out.println("Inventario["+i+"]= "+datosInventario[i]);
                txt_ObjetosAsignados.saveState();
                txt_ObjetosAsignados.beginText();
                txt_ObjetosAsignados.setLineWidth(1.0f);
                txt_ObjetosAsignados.setRGBColorStroke(0x10, 0x10, 0x10);
                txt_ObjetosAsignados.setFontAndSize(bf, 9);
                txt_ObjetosAsignados.setTextMatrix(35, ini);
                txt_ObjetosAsignados.showText(datosInventario[i]);
                txt_ObjetosAsignados.endText();
                txt_ObjetosAsignados.restoreState();
                ini-=13;
            }
    ...
    private void tb_consultaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tb_consultaMouseClicked
        ...
        int filas;
        accion="Modificar";
        filas= tb_consulta.getSelectedRow();

        try {
            //id_actualizar =Id;     
            cargarDatos("nombres",(String)modelo.getValueAt(filas,0));
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(fr_Generacion_Vale_Resguardo_Bienes.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    new fr_Generacion_Vale_Resguardo_Bienes().setVisible(true);
                } catch (ClassNotFoundException ex) {
                    Logger.getLogger(fr_Generacion_Vale_Resguardo_Bienes.class.getName()).log(Level.SEVERE, null, ex);
